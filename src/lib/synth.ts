...
function bell(t: number) {
  const c = getCtx();
  for (const [freq, i] of [[880, 0], [880 * 2.76, 1], [880 * 5.4, 2]] as [number, number][]) {
    const osc = c.createOscillator();
    const g = c.createGain();
    osc.type = "sine";
...
function coin(t: number) {
  const c = getCtx();
  for (const [freq, time] of [[988, 0], [1319, 0.06]] as [number, number][]) {
    const osc = c.createOscillator();
    const g = c.createGain();
    osc.type = "square";
    osc.frequency.value = freq;
...
const GENERATORS: Record<string, (t: number) => void> = {
  kick, snare, hihat, clap, bassDrop, airHorn, laser, boing,
  recordScratch, bell, rimshot, buzzer, coin, powerUp, alarm,
  whoosh, drumroll, blip, explosion, ding, tom, wobble,
};
